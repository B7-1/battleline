...
public class JudgePhase implements Phase {
    JudgePhase(GameSystem s) {
        ...
        BattleLineServer.out_box.println("JudgePhase: ");
    ...
    int alterShield(List<Card> cards, ToIntFunction<List<Card>> func) {
        Card shield = null;
        for (Card c : cards) {
            if (c instanceof UnitCard) continue;	//左辺のオブジェクトが以下の条件のどれかを満たす場合にtrueとなる
    ...
    public void process(GameSystem s, Action act) {
        ...
        s.phase = new DrawPhase(s);		//JudgePhaseの次はDrawPhase
    }
}
